...
const NAV_ITEMS = ["About", "Experience", "Articles", "Skills", "Contact"];
const accent = "#d4896a";
const green = "#7daa6e";
const cyan = "#5bb8d4";
      ...
      <div style={{ maxWidth: 1060, margin: "0 auto", padding: "0 24px" }}>
        ...
        <section style={{ textAlign: "center", padding: "76px 0 56px" }}>
          ...
          <div className="stats-row">
            {STATS.map(function(s, i) { return <div key={i} style={{ background: card, border: "1px solid " + bd, borderRadius: 14, padding: "26px 22px", textAlign: "left" }}><div style={{ width: 44, height: 44, borderRadius: 12, background: s.color, border: "1px solid " + s.border, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 20, marginBottom: 20 }}>{s.icon}</div><div style={{ fontSize: 36, fontWeight: 700, letterSpacing: -0.5, color: tx }}>{s.val}</div><div style={{ color: mt, fontSize: 14, marginTop: 6 }}>{s.label}</div></div>; })}
            <Terminal />
          </div>
        </section>
        ...
        <section id="About" ref={sr("About")} style={{ padding: "80px 0" }}>
          <div className="about-grid">
            <div>
              ...
              <div style={{ display: "flex", flexWrap: "wrap", gap: 8, marginTop: 16, justifyContent: "center" }}>{ABOUT.badgeTags.map(function(t) { return <Tag key={t} a dk={dk}>{t}</Tag>; })}</div>
            </div>
            <div>
              <SLabel>About Me</SLabel>
              <h2 style={{ fontSize: 26, fontWeight: 700, margin: "0 0 22px" }}>{ABOUT.heading}</h2>
              <div style={{ color: mt, lineHeight: 1.85, fontSize: 14.5 }}>{ABOUT.paragraphs.map(function(p, i) { return <p key={i} style={{ marginTop: i > 0 ? 14 : 0 }}>{p}</p>; })}</div>
              {Object.entries(ABOUT.toolCategories).map(function(entry) { var cat = entry[0]; var tools = entry[1]; return <div key={cat} style={{ marginTop: 24 }}><div style={{ fontSize: 11, color: mt, fontWeight: 600, letterSpacing: 2, textTransform: "uppercase", marginBottom: 10 }}>{cat}</div><div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>{tools.map(function(t) { return <Tag key={t} a dk={dk}>{t}</Tag>; })}</div></div>; })}
            </div>
          </div>
        </section>
                  ...
                  <div style={{ background: card, border: "1px solid " + bd, borderRadius: 16, padding: 28, borderTop: i === 0 ? "3px solid " + accent : "1px solid " + bd }}>
                    <div className="exp-header">
                      <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                        ...
                        <div><h3 style={{ margin: 0, fontSize: 18, fontWeight: 700 }}>{e.title}</h3><a href={e.url} target="_blank" rel="noopener noreferrer" style={{ color: accent, fontSize: 14, textDecoration: "none", fontWeight: 500 }}>{e.company} ↗</a></div>
                      </div>
                      <div style={{ textAlign: "right" }}><span style={{ background: "rgba(212,137,106,0.12)", color: accent, padding: "5px 14px", borderRadius: 22, fontSize: 12, fontWeight: 600, border: "1px solid rgba(212,137,106,0.25)" }}>📅 {e.period}</span><div style={{ color: mt, fontSize: 12.5, marginTop: 5 }}>📍 {e.location}</div></div>
                    </div>
                    <p style={{ color: mt, fontSize: 13.5, lineHeight: 1.75, margin: "18px 0" }}>{e.description}</p>
                    <ul style={{ paddingLeft: 0, margin: "14px 0 18px", listStyle: "none" }}>{e.bullets.map(function(b, j) { return <li key={j} style={{ color: mt, fontSize: 13.5, lineHeight: 1.85, marginBottom: 7, paddingLeft: 16, position: "relative" }}><span style={{ position: "absolute", left: 0, color: accent, fontWeight: 700, fontSize: 16, top: -1 }}>•</span>{b}</li>; })}</ul>
                    <div style={{ display: "flex", flexWrap: "wrap", gap: 6, paddingTop: 14, borderTop: "1px solid " + bd }}>{e.tags.map(function(t) { return <Tag key={t} dk={dk}>{t}</Tag>; })}</div>
                  </div>
                </div>
              );
            })}
            ...
            {ARTICLES.map(function(a) {
              return (
                <div key={a.id} onClick={function() { setOpenArticle(a.id); window.scrollTo(0, 0); }} style={{ background: card, border: "1px solid " + bd, borderRadius: 16, padding: 28, cursor: "pointer", borderLeft: "3px solid " + accent, transition: "border-color 0.2s" }}>
                  ...
                  {a.summaryIntro.map(function(p, i) { return <p key={i} style={{ color: mt, fontSize: 14, lineHeight: 1.8, margin: "0 0 8px" }}>{p}</p>; })}
                  <p style={{ color: tx, fontSize: 14, lineHeight: 1.8, margin: "14px 0 10px", fontWeight: 600 }}>{a.summaryHeading}</p>
                  <div style={{ marginBottom: 14 }}>{a.summaryBullets.map(function(b, i) { return <div key={i} style={{ paddingLeft: 16, position: "relative", marginBottom: 6, color: mt, fontSize: 13.5, lineHeight: 1.75 }}><span style={{ position: "absolute", left: 0, color: accent, fontWeight: 700 }}>•</span>{b}</div>; })}</div>
                  <p style={{ color: accent, fontSize: 14, fontWeight: 600, fontStyle: "italic", margin: "0 0 16px" }}>{a.summaryClosing}</p>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 12 }}>
                    <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>{a.tags.map(function(t) { return <Tag key={t} dk={dk}>{t}</Tag>; })}</div>
                ...
                <div key={i} style={{ display: "flex", gap: 24, alignItems: "flex-start" }}>
                  ...
                  <div style={{ background: card, border: "1px solid " + bd, borderRadius: 16, padding: 24, flex: 1 }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 10 }}>
                      ...
                      <div><h3 style={{ margin: 0, fontSize: 16, fontWeight: 700 }}>{e.title}</h3><div style={{ color: accent, fontSize: 13.5, fontWeight: 500 }}>{e.sub}</div></div>
                    </div>
                    <div style={{ color: mt, fontSize: 13.5, marginBottom: 14 }}>📍 {e.location}</div>
                    <div style={{ display: "flex", gap: 8 }}>{e.tags.map(function(t) { return <Tag key={t} a dk={dk}>{t}</Tag>; })}</div>
                  </div>
                </div>
              );
            })}
            ...
            <div>
              <div style={{ background: card, border: "1px solid " + bd, borderRadius: 16, padding: 24 }}>
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "14px 0", borderBottom: "1px solid " + bd }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 12 }}><span style={{ fontSize: 18 }}>✉️</span><span style={{ fontSize: 14, color: tx }}>{PROFILE.email}</span></div>
                  <button onClick={function() { navigator.clipboard.writeText(PROFILE.email); }} style={{ background: "transparent", border: "1px solid " + bd, borderRadius: 8, width: 34, height: 34, cursor: "pointer", color: mt, fontSize: 14, display: "flex", alignItems: "center", justifyContent: "center" }} title="Copy">📋</button>
                </div>
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "14px 0" }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 12 }}><span style={{ fontSize: 18 }}>📞</span><span style={{ fontSize: 14, color: tx }}>{PROFILE.phone}</span></div>
                  <button onClick={function() { navigator.clipboard.writeText(PROFILE.phone); }} style={{ background: "transparent", border: "1px solid " + bd, borderRadius: 8, width: 34, height: 34, cursor: "pointer", color: mt, fontSize: 14, display: "flex", alignItems: "center", justifyContent: "center" }} title="Copy">📋</button>
                </div>
              </div>
        ...
        <footer style={{ borderTop: "1px solid " + bd, padding: "28px 0", display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 16 }}>
          <div><span style={{ fontWeight: 700, fontSize: 16 }}><span style={{ color: accent }}>SDET</span><span style={{ color: mt }}>.</span>Portfolio</span><div style={{ color: mt, fontSize: 12, marginTop: 5 }}>Ensuring quality, reliability, and innovation — one release at a time.</div></div>
          <div style={{ display: "flex", gap: 18 }}>{NAV_ITEMS.map(function(n) { return <button key={n} onClick={function() { go(n); }} style={{ background: "none", border: "none", color: mt, cursor: "pointer", fontSize: 13 }}>{n}</button>; })}</div>
        </footer>
      </div>
    </div>
  );
